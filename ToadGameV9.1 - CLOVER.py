...
class Player(pygame.sprite.Sprite):
    COLOR = (255, 0, 0)
    GRAVITY = 1
    SPRITES = load_sprite_sheets("Character", "Clover", 34, 34, True)
    ANIMATION_DELAY = 4
    ...
    def shoot(self):
        if self.shoot_cooldown == 0:
            ...
            pellet = Bullet(self.rect.x, self.rect.y, 15, 15, self)
    ...
    def update_sprite(self):
        sprite_sheet = "Clover Idle_"
        if self.hit:
            sprite_sheet = "Clover Hit_"
        elif self.y_vel < 0:
            if self.jump_count == 1:
                sprite_sheet = "Clover Jump_"
            elif self.jump_count == 2:
                sprite_sheet = "Clover Double Jump_"
        elif self.y_vel > self.GRAVITY * 2:
            sprite_sheet = "Clover Fall_"
        elif self.x_vel != 0:
            sprite_sheet = "Clover Run_"
            if self.x_vel > 10:
                sprite_sheet = "Clover Dash Left"
            elif self.x_vel < -10:
                sprite_sheet = "Clover Dash Right"
        elif self.shooting == True:
            sprite_sheet = "Clover Attack_"
...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, x, y, width, height, player): #x+y = player's x+y, width+height are 11
        ...
        self.image = pygame.image.load("Clover_bullet.png").convert_alpha()
...
def Grass_level1(window, LEVEL_CLEAR, LEVEL, lives): #drawing test
    ...
    enemies = [parashroom, parashroom2, parashroom3, parashroom4, parashroom5, twiggy, twiggy2, twiggy3, twiggy4, twiggy5, twiggy6]
    ...
    objects = [*floor, Grass_S(0, HEIGHT - block_size * 2, block_size),
               ...
               fire_lily, fire_lily2, lily_vine2, fire_lily3, fire_lily4, lily_vine4, lily_vine4b, lily_vine4c, fire_lily5, lily_vine5,
               lily_vine5b, lily_vine5c, fire_lily6, lily_vine6, lily_vine6b, lily_vine6c, lily_vine6d, twiggy, twiggy2, twiggy3, twiggy4, twiggy5, twiggy6,
...
def Factory_level1A(window, LEVEL_CLEAR, LEVEL, lives): #drawing test
    ...
    Robear = Robobear(900, 460, ENEMY_VEL, 36, 72)

    UFO = NyaFO(2200, 360, ENEMY_VEL, 40, 37)
    ...
    objects = [*floor, Factory_S(0, HEIGHT - block_size * 2, block_size),
               ...
               Factory_S(block_size * 56, HEIGHT - block_size * 2, block_size), orb, orb2, orb3, orb4,
               fplatform, fplatform2, fplatform3, fplatform4, Robear, UFO, goalpost, door]
...
Factory_level1A(window, LEVEL_CLEAR, LEVEL, lives)
